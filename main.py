...
# db.py faylından check_restaurant_status funksiyasını da import edirik
from db import get_restaurant_info, get_restaurant_menu, save_order, check_restaurant_status
...
async def check_active_subscription(restaurant_id: str, update: Update, is_callback: bool = False) -> bool:
    """
    Restoranın abunəlik/sınaq statusunu yoxlayan köməkçi funksiya.
    Status passivdirsə istifadəçiyə xəbərdarlıq mesajı göndərir.
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        # 1. İlk növbədə abunəlik/status yoxlanışı
        ...
        keyboard = [
            [InlineKeyboardButton("🍽️ Menyunu Göstər", callback_data="show_menu")],
...
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if restaurant_id and not await check_active_subscription(restaurant_id, update, is_callback=True):
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_location"):
        ...
        if restaurant_id and not await check_active_subscription(restaurant_id, update):
            return
